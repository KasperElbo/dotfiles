...
| `platforms/*/lib/usage-options.sh`, the `--help` listing and the `--dry-run` lines of each platform's persistent options | `config/install-options.tsv`, and each option's argv arm in `platforms/*/install.sh` | `scripts/render-installer-usage.py` |
