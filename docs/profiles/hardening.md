...
by deleting one file. Four changes are not drop-in files: the `SELINUX=` line
of `/etc/selinux/config`, a marked block in `/etc/security/faillock.conf`
(`pam_faillock` reads no drop-in directory), the `authselect` `with-faillock`
feature, and `dnf5-automatic.timer`. Each of those has its own rollback in
...
| `pam_faillock`: lock an account for 15 minutes after 5 failed password attempts (`authselect enable-feature with-faillock`, and `deny = 5` / `unlock_time = 900` in a block marked `# BEGIN dotfiles` … `# END dotfiles` at the end of `/etc/security/faillock.conf`). `pam_faillock` reads that one file and no `faillock.conf.d`, and a later line for a key replaces an earlier one, so every other line in the file is kept and the block is written last; the previous file is kept as `/etc/security/faillock.conf.dotfiles-<epoch>.bak`. Earlier versions of this profile wrote `/etc/security/faillock.conf.d/90-dotfiles-hardening.conf`, which `pam_faillock` never read, so those machines ran on its built-in 3 attempts / 10 minutes; a rerun removes that file | Fedora ships no lockout at all; mitigates local password guessing against your login/sudo password | `authselect current` (look for `with-faillock`); `sudo faillock --user "$USER"`; `verify-hardening.sh` reads the file as `pam_faillock` does, so a line added after the block, or a `deny=`/`unlock_time=` argument on a `pam_faillock.so` line in `/etc/pam.d/system-auth` or `password-auth` (which `pam_faillock` applies after the file), fails verification | `sudo authselect disable-feature with-faillock`; delete the lines from `# BEGIN dotfiles` through `# END dotfiles` in `/etc/security/faillock.conf` (or restore its backup); `sudo faillock --user "$USER" --reset` clears an active lockout | Mistyping your password 5 times in a row locks the account for 15 minutes |
| sudo audit logfile (`Defaults logfile="/var/log/sudo.log"` in `/etc/sudoers.d/90-dotfiles-hardening`, validated with `visudo -cf` before install) | Fedora's default sudo keeps no dedicated audit trail beyond journald | `sudo -l` (the last `logfile` entry under "Matching Defaults entries" is the one in effect; `verify-hardening.sh` checks it, because a `Defaults !logfile` in a drop-in sorting after this one, or below the `#includedir` line in `/etc/sudoers`, overrides it); `sudo tail /var/log/sudo.log` | Delete the file | None; pure logging addition |
...
| Conservative sshd posture — **only if `sshd` is already active or enabled**: `PermitRootLogin no`, `MaxAuthTries 3`, `LoginGraceTime 20` (`/etc/ssh/sshd_config.d/00-dotfiles-hardening.conf`, validated with `sudo sshd -t` before reload; named `00-` because `sshd` keeps the first value it reads, so it must sort before Fedora's `50-redhat.conf` and the `01-permitrootlogin.conf` Anaconda writes. An earlier install's `90-` copy is removed on the next run) | Fedora Workstation does not install/enable an SSH server, so this profile never installs one just to harden it (per the design constraint); if you've enabled `sshd` yourself, these are small, config-scoped hardenings, not a rewrite of `sshd_config` | `sudo sshd -T` (check `permitrootlogin`, `maxauthtries` and `logingracetime`); `verify-hardening.sh` compares all three against the drop-in, because `sshd` keeps the first value it reads for a keyword and a line above the `Include` in `/etc/ssh/sshd_config`, or a drop-in sorting before `00-dotfiles-hardening.conf`, overrides it | Delete the drop-in, `sudo systemctl reload sshd` | Only relevant if you already run `sshd`; password-only root login and unlimited auth retries stop working, key-based non-root login is unaffected |
...
|---|---|---|
| **Repository-owned** | the `90-dotfiles-hardening` drop-ins under `sysctl.d`, `sudoers.d` and `audit/rules.d`, and `00-dotfiles-hardening.conf` under `sshd_config.d`; the dotfiles block in `/etc/security/faillock.conf`; `authselect`'s `with-faillock` feature; `auditd.service`; `dnf5-automatic.timer` | **Failure.** Each is checked for presence, the exact file mode the installer set, and the exact content it wrote — whole, against the same table the installer generated it from, because a line added above a policy line can reverse the policy while leaving every written line in place. A file can be present and unmodified and still be overruled or never loaded, so where the system can say what it actually applies, that is checked too: the sysctl values with `sysctl -n`, every written audit rule in `auditctl -l`, the sshd directives in `sshd -T` (which keeps the first value it reads), and sudo's `logfile` in `sudo -l` (which keeps the last `Defaults` entry it reads, so a later drop-in's `Defaults !logfile` turns logging off). `authselect current` proves the `with-faillock` feature and `systemctl is-enabled` the services and the timer. Nothing reports what `pam_faillock` resolved, so its values (`deny`, `unlock_time`) are proved by reading `/etc/security/faillock.conf` the way `pam_faillock` does: the dotfiles block must be intact and no later line may override it, and no `pam_faillock.so` line in `/etc/pam.d/system-auth` or `password-auth` may pass `deny=`, `unlock_time=` or `conf=`, which it applies after the file. Reverting one is drift, not a warning. |
...
| **Not observed** | a root-owned drop-in, the loaded audit ruleset, or the sshd or sudo policy in effect, on a machine with no cached `sudo` authorization; `sshd -T` or `sudo -l` failing, or either not answering within `HARDENING_PROBE_TIMEOUT` (30s) | **Not verified**, and named in the output with what to run to check it. Counted separately from a pass, so the summary line says the run was incomplete. |
