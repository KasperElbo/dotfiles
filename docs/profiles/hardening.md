...
| **Not observed** | a root-owned drop-in, the loaded audit ruleset, or the sshd or sudo policy in effect, on a machine with no cached `sudo` authorization; `sshd -T` or `sudo -l` failing, or `sudo -l` not answering within `HARDENING_PROBE_TIMEOUT` (30s) | **Not verified**, and named in the output with what to run to check it. Counted separately from a pass, so the summary line says the run was incomplete. |
