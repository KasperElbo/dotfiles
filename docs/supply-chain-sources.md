...
| Source | Component | Owner | Kind | Privilege | Requested | Resolved | Integrity | Checked before use | Cadence |
|---|---|---|---|---|---|---|---|---|---|
| `github-actions-api` | Workflow runs and branch rules read by the main evidence check | GitHub | `json-api` | `user` | `the runs, jobs and branch rules of this repository` | API response | `https-tls` | no: TLS only | rolling |
...
|---|---|---|---|---|---|
| `github-actions-api` | data | `not-executed` | JSON read by scripts/check-main-evidence.py to report which main commits lack a green run and whether main's rules require the validation jobs; nothing it returns is executed or installed. An API response has no fixed content to pin. | Not executed. | Nothing to review: a wrong or unreadable response makes the report fail or name a commit as missing, and the report changes nothing in the repository. |
...
| `github-actions-api` | `https://api.github.com` | not-applicable | `scripts/check-main-evidence.py` `.github/workflows/main-evidence.yml` `scripts/check-real-install-evidence.py` `.github/workflows/real-install-evidence.yml` |
