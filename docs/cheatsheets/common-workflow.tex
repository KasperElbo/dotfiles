...
Ghostty on Fedora and macOS; Noctty on Windows for the WSL profile, from the same tracked \texttt{shared.conf}. Either owns tabs/splits/zoom locally; tmux does not duplicate that. Copy/paste use the platform's normal shortcut (\texttt{Ctrl+Shift+C/V} on Linux/Wayland, \texttt{Cmd+C/V} on macOS).\par
