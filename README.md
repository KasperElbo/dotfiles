...
| `dnf-automatic-notifyonly.timer` | A developer workstation should not silently install or reboot on a schedule, but knowing updates are available is useful; notify-only downloads metadata and reports, installs nothing | `systemctl is-enabled dnf-automatic-notifyonly.timer` | `sudo systemctl disable --now dnf-automatic-notifyonly.timer` | None; you still update manually, on your own schedule. Skipped if you already run a different `dnf-automatic` timer, so your existing choice is never overridden |
