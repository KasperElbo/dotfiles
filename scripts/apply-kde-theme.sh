...
deprecated_wrapper "scripts/apply-kde-theme.sh" \
  "the theme command, which applies the KDE theme through the Fedora hook" \
  "platforms/fedora/scripts/apply-kde-theme.sh"

exec "$repo_root/platforms/fedora/scripts/apply-kde-theme.sh" "$@"
