...
# Each workstation installer resolves its whole capability selection once and
# shares it between the selection check, preflight and the lifecycle record, so
# assert the always-on baseline and every consumer rather than one literal list
# repeated three times.
for selection in fedora:fedora_selected_capabilities \
  fedora-wsl:wsl_selected_capabilities macos:macos_selected_capabilities; do
  installer="$repo_root/platforms/${selection%%:*}/install.sh"
  resolver="${selection#*:}"
  # shellcheck disable=SC2016 # Matching the literal printf in the installer.
  grep -Fq "printf '%s\\n' base dotnet-debug" "$installer" ||
    fail_test "$installer does not select the supported debugger capability unconditionally"
  [[ "$(grep -Fc "$resolver" "$installer")" -ge 4 ]] ||
    fail_test "$installer does not share one resolved selection between the selection check, preflight and lifecycle state"
done
